...
const siparisGecmisi: Siparis[] = [
  { id: "SP-1041", tarih: "18 Ağu 2026", paket: "Detaylı İç-Dış Yıkama", arac: "Toyota Corolla • 34 ABC 123", durum: "tamamlandi", tutar: "₺420" },
  { id: "SP-1038", tarih: "10 Ağu 2026", paket: "Standart Dış Yıkama", arac: "Toyota Corolla • 34 ABC 123", durum: "tamamlandi", tutar: "₺180" },
  { id: "SP-1052", tarih: "22 Ağu 2026", paket: "Buharlı Temizlik", arac: "Toyota Corolla • 34 ABC 123", durum: "beklemede", tutar: "₺650" },
];

const odemeYontemleri: OdemeYontemi[] = [
...
export default function HesabimSayfasi() {
  ...
  const [form, setForm] = useState<Kullanici>({ ...kullaniciData });
  ...
  const profilAlanlari: { label: string; key: keyof Kullanici; type: string }[] = [
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 rounded-2xl p-6 mb-6 text-white flex items-center gap-5 shadow-lg">
          <div className="w-16 h-16 rounded-full bg-white text-blue-700 flex items-center justify-center text-2xl font-bold shadow">
            {kullaniciData.avatarHarfi}
          </div>
          <div>
            <p className="text-blue-200 text-sm">Hesabım</p>
            <h1 className="text-2xl font-bold">{kullaniciData.ad} {kullaniciData.soyad}</h1>
            <p className="text-blue-100 text-sm mt-0.5">{kullaniciData.email}</p>
          </div>
        </div>
        ...
        {aktifSekme === "profil" && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {profilAlanlari.map(({ label, key, type }) => (
                <div key={key}>
            ...
            <div className="pt-2 flex items-center gap-3">
              <button onClick={handleKaydet} className="bg-blue-700 hover:bg-blue-800 text-white font-semibold px-6 py-3 rounded-xl transition text-sm">
            ...
            {siparisGecmisi.map((s) => (
              <div key={s.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">🚗 {s.arac} · 📅 {s.tarih}</p>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-blue-700 font-bold text-lg">{s.tutar}</span>
                  <button className="text-xs border border-blue-200 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition font-medium">Detay</button>
                </div>
              </div>
            ))}
            <div className="text-center pt-4">
              <a href="/siparis" className="inline-block bg-blue-700 hover:bg-blue-800 text-white font-semibold px-6 py-3 rounded-xl text-sm transition">
                + Yeni Sipariş Ver
              </a>
            </div>
          </div>
        ...
        {aktifSekme === "odeme" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setKartEkleAcik(!kartEkleAcik)} className="text-sm bg-blue-700 hover:bg-blue-800 text-white font-semibold px-4 py-2 rounded-xl transition">
                + Kart Ekle
              </button>
            </div>
            {odemeYontemleri.map((k) => (
              <div key={k.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-semibold text-slate-800 text-sm">
                      {k.tip} •••• {k.son4}
                      {k.varsayilan && <span className="ml-2 text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-semibold">Varsayılan</span>}
                    </p>
                    <p className="text-xs text-slate-400">{k.marka}</p>
                  </div>
                </div>
                <button className="text-xs text-red-400 hover:text-red-600 font-medium transition">Kaldır</button>
              </div>
            ))}
